...
def multi_qr_code_generator(n,d,sig,scale):
    multi_qr_data = np.zeros((d, n*scale,n*scale))
    multi_qr_noise = np.zeros((d, n*scale,n*scale))
    for l in range(d):
        multi_qr_noise[l,:,:], multi_qr_data[l,:,:] = qr_code_generator(n,sig,scale)
    return multi_qr_noise, multi_qr_data

def color_qr_code_generator(n,sig,scale):
    multi_qr_data = np.zeros((4, n*scale,n*scale))
    multi_qr_noise = np.zeros((4, n*scale,n*scale))
    for i in range(n):
        for j in range(n):
            index = np.random.randint(0, 4)
            multi_qr_data[index,scale*i:scale*(i+1),scale*j:scale*(j+1)] = 1
    multi_qr_noise = multi_qr_data + sig*np.random.randn(4, n*scale,n*scale)
    return multi_qr_noise, multi_qr_data
...
def cmyk_to_rgb_image(cmyk_image):
    rgb_image = np.zeros_like(cmyk_image[:, :, :3])  # Initialize RGB array
    for i in range(cmyk_image.shape[0]):
        for j in range(cmyk_image.shape[1]):
            c, m, y, k = cmyk_image[i, j]
            rgb_image[i, j] = cmyk_to_rgb(c, m, y, k)
    return rgb_image
...
def ADMM_TV_BOX_approx(y, y0, iter, lam, rho, eps):
    ...
    Z1 = np.zeros((N-1, N, 3,3))
    Z2 = np.zeros((N, N-1, 3,3))

    data = np.zeros(iter)
    datares1 = np.zeros(iter)
    datares2 = np.zeros(iter)
    datares3 = np.zeros(iter)
    datatime = np.zeros(iter)
    flagggg = 0
    ...
    for i in range(iter):
        ...
        [ux, ur1, ur2] = adjL_1R_image(U1, U2, N)
        [zx, zr1, zr2] = adjL_1R_image(Z1, Z2, N)
        ...
        [L1, L2] = L_1R_image(x, r1, r2, N)

        UU1 = U1
        UU2 = U2

        [U1, U2] = ADMMprox_image(L1 + Z1, L2 + Z2, N)

        Z1 = Z1 + L1 - U1
        Z2 = Z2 + L2 - U2
        
        data[i] = np.sum(1 - np.real(x*y)) + lam*np.sum(1 - r1) + lam*np.sum(1 - r2)
        res1, res2, res3 = adjL_1R_image(UU1 - U1, UU2 - U2, N)
        ...
        datares3[i] = 1 - np.mean(np.sqrt(np.real(x)**2))
        datares3[i] = np.mean(np.abs(1 - np.sqrt(np.real(x)**2)))
        datatime[i] = time.time()
    ...
    return [x, data, datares1, datares2, datares3, datatime-starttime]
...
def ADMM_TV_CUBE_2_image(y, y0, mu, rho, gam, ord):

        k, d, d = np.shape(y)

        scale = []
        for j in range(k):
            scale = np.append(scale, 2**j)
        scale = np.array(scale)

        if ord == 1:
            c = -1/2*(np.abs(1 - y) - np.abs(1+y))
        if ord == 2:
            c = y
        ...
        x1 = np.random.randn(k,d,d)

        j = 0

        delta = np.linalg.norm(1 - np.abs(x))
        d1 = np.linalg.norm(1 - np.abs(x))
        d2 = np.linalg.norm(1 - np.abs(x))/np.size(x[0,0,:])**2/k
        ...
        while d2 > 1e-4:
            ...
            #x = condat_tv.tv_denoise_matrix(u - z + y/rho, mu/rho)
            for i in range(k):
                # x[i,:,:] = ADRA(u[i,:,:] - z[i,:,:] + c[i,:,:]/rho, gam, 2**i*mu/rho, 100)
                x[i,:,:] = ADRA(u[i,:,:] - z[i,:,:] + c[i,:,:]/rho, gam, mu/rho, 100)

            # proj_B(1)
            for i in range(k):
                u[i,:,:] = proj_B1_image(x[i,:,:] + z[i,:,:])
            ...
            if np.mod(j,50) == 0:
                print(j, ' \t| ', np.sum(np.sum(np.sum((x - y)**2,2),1)*scale) + mu*np.sum(np.sum(np.sum(np.diff(x, axis=2),2),1)*scale) + mu*sum(np.sum(np.sum(np.diff(x, axis=1),2),1)*scale) , ' \t| ', "%10.2e"% (np.linalg.norm(x - y0)**2), ' \t| ',  "%10.2e"% (np.sum(np.abs(x - y0))), ' \t| ' , "%10.2e"% (d2), ' \t|', "%10.2e"% s)
        ...
        s = 0
        #for i in range(k):
        #    ss = ss + MIoU(y0[i,:,:], np.sign(x[i,:,:]))
        #    s = s + np.nanmean(ss)
        
        print(j, ' \t| ', np.sum(np.sum(np.sum((np.sign(x) - y)**2,2),1)*scale) + mu*sum(np.sum(np.sum(np.diff(np.sign(x), axis=2),2),1)*scale) + mu*sum(np.sum(np.sum(np.diff(np.sign(x), axis=1),2),1)*scale) , ' \t| ', "%10.2e"% (np.sqrt(np.sum(np.sign(x) - y0)**2)), ' \t| ',  "%10.2e"% (np.sum(np.abs(np.sign(x) - y0))), ' \t| ' , "%10.2e"% (d2), ' \t|', "%10.2e"% s)

        datei = open('data_AD_2Dgrid_cube_2_tv.txt','a')
        datei.write(str(np.sqrt(np.sum((np.sign(x) - y0)**2))))
        datei.write('\n')
        datei.write(str(np.sum(np.abs(np.sign(x) - y0))))
        datei.write('\n')
...
def ADMM_TV_CUBE_image(y, y0, mu, rho, gam, iter):
        ...
        delta = np.linalg.norm(1 - np.abs(x))
        d1 = np.linalg.norm(1 - np.abs(x))/np.size(x[0,0,:])**2

        #while np.linalg.norm(x1 - x) > 1e-3:
        while d1 > 1e-4 and j < iter:
            x1 = x
            ...
            for i in range(k):
                # x[i,:,:] = ADRA(u[i,:,:] - z[i,:,:] + c[i,:,:]/rho, gam, 2**i*mu/rho, 100)
                x[i,:,:] = ADRA(u[i,:,:] - z[i,:,:] + y[i,:,:]/rho, gam, mu[i]/rho, 100)
            ...
            s = 0
            for i in range(k):
                s += np.nanmean(MIoU(y0[i,:,:], np.sign(x[i,:,:])))

            delta = d1 - np.linalg.norm(1 - np.abs(x))
            d1 = np.linalg.norm(1 - np.abs(x))/np.size(x[0,0,:])**2
        
            if np.mod(j,50) == 0:
                print(j, ' \t| ', np.linalg.norm(x - y)**2 + np.sum(mu*np.sum(np.sum(np.diff(x, axis=2),2),1)) + sum(mu*np.sum(np.sum(np.diff(x, axis=1),2),1)) , ' \t| ', "%10.2e"% (np.linalg.norm(x - y0)**2), ' \t| ',  "%10.2e"% (np.sum(np.abs(x - y0))), ' \t| ' , "%10.2e"% (d1), ' \t| ' , "%10.2e"% s)
        ...
        print(j, ' \t| ', np.sum(np.sum(np.sum((np.sign(x) - y)**2,2),1)) + sum(mu*np.sum(np.sum(np.diff(np.sign(x), axis=2),2),1)) + sum(mu*np.sum(np.sum(np.diff(np.sign(x), axis=1),2),1)) , ' \t| ', "%10.2e"% (np.sqrt(np.sum(np.sign(x) - y0)**2)), ' \t| ',  "%10.2e"% (np.sum(np.abs(np.sign(x) - y0))), ' \t| ' , "%10.2e"% (d1), ' \t| ' , "%10.2e"% s)

        datei = open('data_AD_2Dgrid_cube_tv.txt','a')
        datei.write(str(np.sqrt(np.sum((np.sign(x) - y0)**2))))
        datei.write('\n')
        datei.write(str(np.sum(np.abs(np.sign(x) - y0))))
        datei.write('\n')
...
def mulit_qr_code_plt(x,y,z):
    fig = plt.figure(0,figsize=(20,5), dpi = 4*100)
    plt.rc('font', size=7.5) 

    ax1 = fig.add_subplot(1, 4, 1)
    ax1.imshow(np.transpose(x,(1,2,0)))

    ax = fig.add_subplot(1, 4, 2)
    ax.imshow(np.transpose(y,(1,2,0)))

    ax2 = fig.add_subplot(1, 4, 3)
    ax2.imshow(np.transpose(z,(1,2,0)))

    ax3 = fig.add_subplot(1, 4, 4)
    ax3.imshow(np.transpose(x,(1,2,0)) - np.transpose(z,(1,2,0)))
    #plt.savefig("0D_2D_solution_2.pdf",dpi=400)


def color_qr_code_plt(x,y,z):
    fig = plt.figure(0,figsize=(20,5), dpi = 4*100)
    plt.rc('font', size=7.5) 
